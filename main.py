...
while not weapon:
    ...
    if char_weapon.lower() in ["sword", "axe", "staff", "bow"]:  # checking against list of weapons
        ...
        inventory.append(char_weapon)  # add wep to 1st inventory slot
...
def level_up(character_stats, experience, next_level, xp_gain):  # take 3 arguments, stats, current xp and next level xp
    global strength, magic, dexterity

    print(f"You gained: {xp_gain} experience")
    print(f"Current experience: {experience}")
    print(f"Experience needed to level up: {next_level}")
    ...
    if experience >= next_level:
        ...
        print(f"Current experience: {experience}")
        print(f"Experience needed for next level: {next_level}")
        return character_stats[:], strength, magic, dexterity, next_level
    else:
        print(f"Current experience: {experience}")
        print(f"Experience needed for next level: {next_level}")
        return character_stats[:], strength, magic, dexterity, next_level
...
def trap(dexterity, experience, next_level):
    ...
    if dodge_roll >= dodge_chance:  # calculate chances of dodging trap
        ...
        # experience += xp_gain
        # level_up(character_stats, experience, next_level, xp_gain) #check levelup
        return trap_dodged
    else:
        trap_dodged = False
...
while adventure:
    ...
    while health_points > 0 and goblin.health > 0:
        ...
        # players turn
            ...
            goblin.health -= player_damage  # deal damage to enemy
            if goblin.health <= 0:
                print("You slayed that goblin bish")
                experience = experience + xp_gain
                print(f"You gained: {xp_gain} experience")
                level_up(character_stats, experience, next_level, xp_gain)  # call levelup
...
# initiate trap
dodge_trap = trap(dexterity, experience, next_level)  # call trap function
if not dodge_trap:
    print("GAME OVER")
...
if answer == "yes":
    ...
    while health_points > 0 and troll.health > 0 and not progress:
        ...
        if troll.health <= 0:
            ...
            experience = experience + troll.xp_gain  # gain experience
            print(f"You gained {troll.xp_gain} experience, your total experience is now: {experience}")
            level_up(character_stats, experience, next_level, troll.xp_gain)  # call levelup
            health_points = character_stats[0]  # reset health
            progress = True
...
# chapter 3
print("You loot the body of the Troll")
print("You take the Troll's club and you also find an ancient magik tome containing Fireball spell")


inventory.append("Club")
inventory.append("Fireball")
print("Your current inventory holds: ", inventory[:])
